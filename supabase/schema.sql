...
-- ---------------------------------------------------------------------
-- Bons de commande + rapprochement facture↔commande (cf. migration_commandes.sql)
-- ---------------------------------------------------------------------
create table if not exists public.commandes (
  id uuid primary key default gen_random_uuid(),
  org_id uuid not null references public.organisations(id) on delete cascade,
  fournisseur_id uuid references public.fournisseurs(id) on delete set null,
  numero text, date date, devise text not null default 'XOF',
  total_ht numeric(14,2) not null default 0,
...
create table if not exists public.commandes_lignes (
  ...
  designation text, quantite numeric(14,3) not null default 0,
  prix_unitaire numeric(14,2) not null default 0,
  montant_ht numeric(14,2) not null default 0,
  created_at timestamptz not null default now()
);
...
alter table public.factures add column if not exists commande_id uuid references public.commandes(id) on delete set null;
...
create policy cmd_select on public.commandes for select using (org_id = public.current_org_id());
drop policy if exists cmd_write on public.commandes;
create policy cmd_write on public.commandes for all
...
create policy cmd_lignes_select on public.commandes_lignes for select using (exists (
  select 1 from public.commandes c where c.id = commandes_lignes.commande_id and c.org_id = public.current_org_id()));
drop policy if exists cmd_lignes_write on public.commandes_lignes;
create policy cmd_lignes_write on public.commandes_lignes for all
  using (exists (select 1 from public.commandes c where c.id = commandes_lignes.commande_id and c.org_id = public.current_org_id()
    and public.current_role()::text in ('admin','saisie','controle_gestion')))
...
-- Factures récurrentes / abonnements (cf. migration_recurrences.sql)
create table if not exists public.recurrences (
  ...
  montant_ht numeric(14,2) not null default 0,
  taux_tva numeric(5,2) not null default 18,
  categorie text, devise text not null default 'XOF',
  jour smallint not null default 1 check (jour between 1 and 28),
  actif boolean not null default true,
  date_debut date, date_fin date, derniere_periode text,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  created_by uuid references auth.users(id)
);
...
create policy rec_select on public.recurrences for select using (org_id = public.current_org_id());
...
-- Rapprochement bancaire / lettrage (cf. migration_banque.sql)
create table if not exists public.operations_bancaires (
  id uuid primary key default gen_random_uuid(),
  org_id uuid not null references public.organisations(id) on delete cascade,
  date date, libelle text, montant numeric(14,2) not null default 0,
  sens text not null default 'debit' check (sens in ('debit','credit')),
  reference text, lettree boolean not null default false,
...
create policy op_select on public.operations_bancaires for select using (org_id = public.current_org_id());
drop policy if exists op_write on public.operations_bancaires;
create policy op_write on public.operations_bancaires for all
  using (org_id = public.current_org_id() and public.current_role()::text in ('admin','saisie','controle_gestion'))
  with check (org_id = public.current_org_id() and public.current_role()::text in ('admin','saisie','controle_gestion'));

-- Lettrage multi-factures / allocations (cf. migration_lettrages.sql)
...
create policy let_select on public.lettrages for select using (org_id = public.current_org_id());
drop policy if exists let_write on public.lettrages;
create policy let_write on public.lettrages for all
  using (org_id = public.current_org_id() and public.current_role()::text in ('admin','saisie','controle_gestion'))
  with check (org_id = public.current_org_id() and public.current_role()::text in ('admin','saisie','controle_gestion'));
